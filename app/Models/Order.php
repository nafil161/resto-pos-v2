...
class Order extends Model
{
    ...
    {
        return match ($this->pos_payment_method) {
            \App\Enums\PosPaymentMethod::CASH => 'Cash',
            \App\Enums\PosPaymentMethod::CARD => 'Card',
            \App\Enums\PosPaymentMethod::MOBILE_BANKING => 'UPI',
            default => 'Unknown',
        };
    }

    public static function getPaymentTypeId($type)
    {
        // Inside your Controller or the Route fetching the Order
        $paymentMethods = [
            'Cash' => PosPaymentMethod::CASH,
            'Card' => PosPaymentMethod::CARD,
            'UPI' => PosPaymentMethod::MOBILE_BANKING,

        ];

        return $paymentMethods[$type] ?? 1;
    }

    public function changeBackToUser()
    {
        if ($this->pos_payment_method != PosPaymentMethod::CASH) {
            return null;
        }

        return $this->pos_received_amount - $this->total;
    }
}
